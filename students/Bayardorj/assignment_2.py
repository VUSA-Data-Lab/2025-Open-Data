...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 30 21:16:32 2025
...
italy_clean['population_pct_change'] = (italy_clean['population'].pct_change() * 100)
...
total_change = italy_clean['population'].iloc[-1] - italy_clean['population'].iloc[0]
pct_change_total = ((italy_clean['population'].iloc[-1] - italy_clean['population'].iloc[0]) / italy_clean['population'].iloc[0]) * 100
...
fig.suptitle('Italy Population Analysis: Demographic Decline Crisis', fontsize=16, fontweight='bold')

# Plot 1: Population Trend
axes[0,0].plot(italy_clean['year'], italy_clean['population'], 
               linewidth=2.5, marker='o', markersize=4, label='Actual Population')
axes[0,0].plot(italy_clean['year'], italy_clean['population_5yr_ma'], 
               linewidth=2, linestyle='--', alpha=0.8, label='5-Year Moving Average')
...
axes[0,0].annotate(f'Peak: {italy_clean.loc[peak_idx, "population"]:,.0f}', 
                   xy=(italy_clean.loc[peak_idx, 'year'], italy_clean.loc[peak_idx, 'population']),
                   xytext=(10, 10), textcoords='offset points',
                   bbox=dict(boxstyle='round,pad=0.3', facecolor='red', alpha=0.1),
                   arrowprops=dict(arrowstyle='->', color='red'))

# Plot 2: Annual Population Change
axes[0,1].bar(italy_clean['year'], italy_clean['population_change'], 
              color=['red' if x < 0 else 'green' for x in italy_clean['population_change']],
              alpha=0.7)
...
axes[1,0].plot(italy_clean['year'], italy_clean['population_pct_change'], 
               linewidth=2, marker='s', markersize=4, color='orange')
axes[1,0].axhline(y=0, color='black', linestyle='-', alpha=0.3)
axes[1,0].fill_between(italy_clean['year'], italy_clean['population_pct_change'], 
...
axes[1,1].fill_between(italy_clean['year'], italy_clean['change_from_peak'], 
                       color='red', alpha=0.6)
axes[1,1].plot(italy_clean['year'], italy_clean['change_from_peak'], 
...
combined_data = pd.concat([italy_clean[['year', 'population']], projection_df], ignore_index=True)
combined_data['type'] = ['Actual'] * len(italy_clean) + ['Projected'] * len(projection_df)
...
plt.plot(actual_data['year'], actual_data['population'], 
         linewidth=2.5, marker='o', label='Actual', color='blue')
plt.plot(projected_data['year'], projected_data['population'], 
         linewidth=2.5, linestyle='--', marker='s', label='Projected', color='red')

plt.title('Italy Population Trend with 10-Year Projection', fontsize=14, fontweight='bold')
plt.xlabel('Year')
...
plt.annotate(f'Projected 2035: {projected_2035:,.0f}\nAdditional decline: {total_decline:,.0f}', 
             xy=(2035, projected_2035), xytext=(2028, projected_2035 + 500000),
             bbox=dict(boxstyle='round,pad=0.3', facecolor='red', alpha=0.1),
             arrowprops=dict(arrowstyle='->', color='red'))
...
print("ITALY POPULATION CRISIS - KEY FINDINGS")
print("="*50)
