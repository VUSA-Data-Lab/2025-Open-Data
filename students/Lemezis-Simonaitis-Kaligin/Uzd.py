...
def main():
    ...
    status_counts = df.groupby('problemos_tipas')[
        'problemos_statusas'].value_counts()
    ...
    df['problem_reg_data'] = pd.to_datetime(
        df['problem_reg_data'], errors='coerce')
    df['problem_issprend_data'] = pd.to_datetime(
        df['problem_issprend_data'], errors='coerce')

    # Naujas stulpelis
    df['days_to_solve'] = (
        df['problem_issprend_data'] -
        df['problem_reg_data']).dt.days

    # Naudoja nauja stulpeli, kad paskaiciuoti per kiek vidutiniskai laiko,
    # problemos buvo issprestos
    avg_days = df.groupby('problemos_tipas')[
        'days_to_solve'].mean().sort_values(ascending=False)
